...
with col2:
    ...
    if not filtered_data.empty:
        ...
        st.metric(label="**Precipitaciones Medias**", value=f"{(filtered_data['media_prec'].round(2).iloc[0]):.2f} mm", delta=f"{(filtered_data['media_prec'].round(2).iloc[0] - filtered_data_previous['media_prec'].round(2).iloc[0]):.2f} mm")
        st.metric(label="**Audiencia Media**", value=f"{(filtered_data['sessions'].round(2).iloc[0] / 1000000):.2f} M", delta=f"{((filtered_data['sessions'].round(2).iloc[0] - filtered_data_previous['sessions'].round(2).iloc[0]) / 1000000):.2f} M")
